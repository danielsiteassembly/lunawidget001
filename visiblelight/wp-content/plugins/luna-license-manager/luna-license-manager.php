...
final class VL_License_Manager {
    ...
    private function __construct() {
        ...
        add_filter('rest_authentication_errors', array($this, 'allow_public_rest_access'), 10, 1);
        add_filter('rest_pre_serve_request', array($this, 'ensure_json_response'), 10, 4);
        add_action('rest_api_init', array($this, 'add_global_cors_headers'), 0);
...
<?php endforeach; ?>
     ...
     * CRITICAL: VL Clients should NEVER access wp-admin except for VL Hub admin pages.
     */
    public function enforce_vl_client_post_auth_redirect() {
        ...
        $user = wp_get_current_user();
        if (!($user instanceof WP_User) || !in_array('vl_client', (array) $user->roles, true)) {
            return; // Not a VL client, allow normal wp-admin access
        }
        ...
        // VL Client is trying to access wp-admin but NOT a VL Hub admin page
        // ALWAYS redirect to Supercluster - NEVER allow wp-admin access for VL Clients
        $license_key = get_user_meta($user->ID, 'vl_license_key', true);
        if (self::is_legacy_license_key($license_key)) {
            delete_user_meta($user->ID, 'vl_license_key');
            delete_user_meta($user->ID, 'license_key');
        ...
        error_log('[VL Client Redirect] VL Client ' . $user->user_login . ' attempted to access wp-admin. Redirecting to Supercluster: ' . $url);
        wp_safe_redirect($url);
        exit;
    }
    ...
    /**
     * Allow public (unauthenticated) access to vl-hub, vl-license, and luna_widget REST API endpoints.
     * This bypasses WordPress's default authentication requirement for these specific namespaces.
     * 
     * @param WP_Error|null $result Error from another authentication handler, null if no error.
     * @return WP_Error|null Returns null to allow access, or the original error for other endpoints.
     */
    public function allow_public_rest_access($result) {
        // Only process if we're in a REST API request
        ...
        // Check if this is a vl-hub, vl-license, or luna_widget endpoint
        $is_our_endpoint = false;
        if (!empty($route)) {
            $is_our_endpoint = (
                strpos($route, '/wp-json/vl-hub/') !== false || 
                strpos($route, '/wp-json/vl-license/') !== false ||
                strpos($route, '/wp-json/luna_widget/') !== false
            );
        }
        
        // If this is one of our endpoints, allow unauthenticated access
        if ($is_our_endpoint) {
            // Return null to indicate no authentication error (allow access)
            // This works whether $result is null (no error) or a WP_Error (authentication error)
            return null;
        }
        
        // For all other endpoints, return the original result (which may require authentication)
        return $result;
    }
    
    /**
     * Add CORS headers globally for all REST API requests.
     * This ensures cross-origin requests from supercluster.visiblelight.ai work properly.
     */
    public function add_global_cors_headers() {
        // Only add CORS headers for REST API requests
    ...
    /**
     * Ensure REST API responses are always JSON, even on errors.
     * This prevents HTML error pages from breaking JSON parsing in the frontend.
     * 
     ...
     */
    public function ensure_json_response($served, $result, $request, $server) {
        // Only process if we're in a REST API request and headers haven't been sent
        if (!defined('REST_REQUEST') || !REST_REQUEST || headers_sent()) {
            return $served;
        }
    ...
    public function register_rest_routes() {
        ...
        register_rest_route(
            ...
            array(
                'methods'             => 'GET',
                'permission_callback' => '__return_true',
                'callback'            => array($this, 'rest_client_sites'),
            )
        );
    ...
    /**
     * REST handler: return all sites/apps assigned to a client license.
     */
    public function rest_client_sites($request) {
        $this->add_cors_headers();
        ...
        // Get all assigned sites for this license
        $assigned_sites = isset($license_record['assigned_sites']) && is_array($license_record['assigned_sites']) 
            ? $license_record['assigned_sites'] 
            : array();
        
        // Always include the primary site if it exists
        if (!empty($license_record['site']) && !in_array($license_record['site'], $assigned_sites, true)) {
            $assigned_sites[] = $license_record['site'];
        }
        
        // If no assigned sites and no primary site, return empty array
        ...
        // Fetch metadata for each site (og:image, title, SSL/TLS status, ping status)
        $sites_data = array();
        foreach ($assigned_sites as $site_url) {
            $site_data = array(
                'url' => $site_url,
            ...
            // Fetch site metadata (og:image, title)
            $site_metadata = $this->fetch_site_metadata($site_url);
            if ($site_metadata) {
                $site_data['og_image'] = $site_metadata['og_image'] ?? null;
                $site_data['title'] = $site_metadata['title'] ?? null;
            }
            
            // Check SSL/TLS status
            $ssl_status = $this->check_ssl_status($site_url);
            $site_data['ssl_status'] = $ssl_status;
            
            // Ping site
            $ping_result = $this->ping_site($site_url);
            if ($ping_result['success']) {
                $site_data['ping_status'] = 'active';
                $site_data['ping_label'] = 'Active';
                $site_data['ping_color'] = '#00a32a';
            } else {
                $site_data['ping_status'] = 'down';
                $site_data['ping_label'] = 'Down';
                $site_data['ping_color'] = '#d63638';
            }
    ...
    /**
     * Fetch site metadata (og:image, title) from a URL.
     */
    private function fetch_site_metadata($url) {
        $transient_key = 'vl_site_metadata_' . md5($url);
        ...
        // Use wp_remote_get to fetch the page
        $response = wp_remote_get($url, array(
            'timeout' => 10,
            'sslverify' => false,
            'user-agent' => 'Visible Light Hub/1.0'
        ));
        ...
        if (preg_match('/<meta\s+property=["\']og:image["\']\s+content=["\']([^"\']+)["\']/i', $body, $matches)) {
            $metadata['og_image'] = esc_url_raw($matches[1]);
        }
        ...
        // Cache for 1 hour
        set_transient($transient_key, $metadata, HOUR_IN_SECONDS);
        
        return $metadata;
    }
    
    /**
     * Check SSL/TLS status for a URL.
     */
    private function check_ssl_status($url) {
        ...
        $transient_key = 'vl_ssl_status_' . md5($host . ':' . $port);
        $cached = get_transient($transient_key);
        if ($cached !== false) {
            return $cached;
        }
        ...
        if ($scheme !== 'https') {
            $status = 'not_https';
            set_transient($transient_key, $status, 30 * MINUTE_IN_SECONDS);
            return $status;
        }
        ...
        if ($socket) {
            ...
            if (isset($params['options']['ssl']['peer_certificate'])) {
                ...
                $valid_until = isset($cert['validTo_time_t']) ? $cert['validTo_time_t'] : 0;
                $now = time();
                
                if ($valid_until > $now) {
                    $days_until_expiry = floor(($valid_until - $now) / DAY_IN_SECONDS);
                    $status = $days_until_expiry > 30 ? 'valid' : ($days_until_expiry > 0 ? 'expiring_soon' : 'expired');
                } else {
                    $status = 'expired';
                }
            } else {
    ...
    /**
     * Ping a site to check if it's accessible.
     */
    private function ping_site($url) {
        $transient_key = 'vl_ping_' . md5($url);
        $cached = get_transient($transient_key);
        if ($cached !== false) {
            return $cached;
        }
        ...
        if (is_wp_error($response)) {
            ...
            $result['success'] = $status_code >= 200 && $status_code < 400;
        }
    ...
    public function render_hub_profile_screen() {
        ...
        // Handle Client Syncing form submission
        if (isset($_POST['action']) && $_POST['action'] === 'assign_site_to_client') {
            check_admin_referer('vl_client_syncing');
            
            $client_license = isset($_POST['sync_client_license']) ? sanitize_text_field(wp_unslash($_POST['sync_client_license'])) : '';
            $site_url = isset($_POST['sync_site_url']) ? esc_url_raw(wp_unslash($_POST['sync_site_url'])) : '';
            
            if ($client_license && $site_url && isset($licenses[$client_license])) {
                $store = self::lic_store_get();
                if (isset($store[$client_license])) {
                    // Initialize assigned_sites array if it doesn't exist
                    if (!isset($store[$client_license]['assigned_sites']) || !is_array($store[$client_license]['assigned_sites'])) {
                        $store[$client_license]['assigned_sites'] = array();
                        // Add primary site if it exists
                        if (!empty($store[$client_license]['site'])) {
                            $store[$client_license]['assigned_sites'][] = $store[$client_license]['site'];
                        }
                    }
                    
                    // Add site if not already assigned
                    if (!in_array($site_url, $store[$client_license]['assigned_sites'], true)) {
                        $store[$client_license]['assigned_sites'][] = $site_url;
                        $store[$client_license]['last_updated'] = current_time('mysql');
                        
                        self::lic_store_set($store);
                        $messages['success'][] = 'Site/app assigned to client successfully.';
                        
                        // Update licenses array for display
                        $licenses = $store;
                        if ($selected_license === $client_license) {
                            $selected_license = $client_license; // Refresh selected license
                        }
                    } else {
                        $messages['error'][] = 'This site/app is already assigned to this client.';
                    }
                }
            } else {
                $messages['error'][] = 'Please select both a client and a site/app.';
            }
        }
            ...
            <div class="vl-hub-tabs" style="margin-top: 20px;">
                ...
                <?php if ($selected_license && isset($licenses[$selected_license])) : ?>
                    ...
                    <div class="vl-client-profile">
                        ...
                        <!-- Client Syncing Section -->
                        <div style="margin-top: 30px; padding: 20px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 5px;">
                            <h3 style="margin-top: 0;">Client Syncing</h3>
                            <p class="description">Assign multiple sites or apps to a client's VL Hub account. This allows clients to manage multiple properties under one license.</p>
                            
                            <form method="post" id="client-syncing-form" style="margin-top: 15px;">
                                <?php wp_nonce_field('vl_client_syncing'); ?>
                                <input type="hidden" name="action" value="assign_site_to_client">
                                
                                <table class="form-table">
                                    <tr>
                                        <th scope="row"><label for="sync_client_select">Choose Client</label></th>
                                        <td>
                                            <select id="sync_client_select" name="sync_client_license" required style="min-width: 300px;">
                                    ...
                                    <tr>
                                        <th scope="row"><label for="sync_site_select">Assign Site or App</label></th>
                                        <td>
                                            <select id="sync_site_select" name="sync_site_url" required style="min-width: 300px;">
                                                <option value="">-- Select a site/app --</option>
                                                <?php 
                                                // Get all unique sites from all licenses
                                                $all_sites = array();
                                                foreach ($licenses as $license_key => $license) {
                                                    if (!empty($license['site'])) {
                                                        $all_sites[$license['site']] = $license['site'];
                                                    }
                                                }
                                                foreach ($all_sites as $site_url) : ?>
                                                    <option value="<?php echo esc_attr($site_url); ?>">
                                                        <?php echo esc_html($site_url); ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                            <p class="description">Select an existing site/app to assign to the chosen client. If the site doesn't exist, create a new license first.</p>
                                        </td>
                                    </tr>
                                </table>
                                
                                <p class="submit">
                                    <input type="submit" class="button-primary" value="Assign Site to Client" />
                                </p>
                            </form>
                            
                            <?php
                            // Display currently assigned sites for selected client
                            if ($selected_license && isset($licenses[$selected_license])) {
                                $assigned_sites = isset($licenses[$selected_license]['assigned_sites']) ? $licenses[$selected_license]['assigned_sites'] : array();
                                if (empty($assigned_sites) && !empty($licenses[$selected_license]['site'])) {
                                    // Include the primary site
                                    $assigned_sites = array($licenses[$selected_license]['site']);
                                }
                                if (!empty($assigned_sites)) :
                            ?>
                                <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #ddd;">
                                    <h4>Currently Assigned Sites/Apps for <?php echo esc_html(isset($licenses[$selected_license]['client_name']) ? $licenses[$selected_license]['client_name'] : 'This Client'); ?>:</h4>
                                    <ul style="list-style: disc; margin-left: 20px;">
                                        <?php foreach ($assigned_sites as $site) : ?>
                                            <li><?php echo esc_html($site); ?></li>
                                        <?php endforeach; ?>
                                    </ul>
                                </div>
                            <?php endif; } ?>
